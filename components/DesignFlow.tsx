...
import { GeneratorStep, HennaStyle, HandAnalysis, OutfitAnalysis, SavedDesign } from '../types';
...
const INITIAL_STYLES: HennaStyle[] = [
  {
    id: 'regal-bloom',
    name: 'Regal Bloom',
  ...
  },
  {
    id: 'sunset-paisley',
    name: 'Sunset Paisley',
    description: 'Sunset-inspired motifs with rich, bold paisley elements.',
    imageUrl: 'https://lh3.googleusercontent.com/aida-public/AB6AXuC0r0MxPzfwpY-SBNfLHnqBP0jX4TiN5u10P37na4yofszbNlEGpBUXamGzNnnt1tqdFsQzEObqdZZXEB7GtslFft_YDqyyggRzQB-O052DQrfDU-oNQFm6dPyEahORwqV-CmJHvaQk_TR_b9XGPvmpwNxQqyvzzuoUB-4eAoHgFAsgl_hzOc-5WMdkklCyvTK1HD0eDw4-LujGvl02RyRfG5PL5XcXbehqsgzXHRLU4_nuIcqbWbhDaG5gWC1JwgdyrEt6l-hbUg',
    promptModifier: 'sunset paisley style, paisley motifs, bold shading, traditional Indian patterns, dense design'
  },
  {
    id: 'ethereal-lace',
    name: 'Ethereal Lace',
    description: 'Delicate, lace-like meshwork for a lighter, airy feel.',
    imageUrl: 'https://lh3.googleusercontent.com/aida-public/AB6AXuDSiHatrYm9qXj7ToUzPiwFYa7jfzBJmoiBQGBwJUXv5tpLWMDESR0B2M_T_TJRFTnbkTKwpMY4XhWMlI9XfKXVGiWAPBZQa5dtSLc52A1kCs7XXNMCSgMOBJZ1AUPpfNVeRzpj_4U-427W0bII29_S2MxWucUp8NTucCHJkjECghIVwHJ3jDpoE78qHYvaUBDs9PBZ7UQtdnhK24ovgWQMgDUwObC0k1Cg_Iqzk6-GBv10UWGbzLsGw1rktoO6UfBJbPEqx3pcyw',
    promptModifier: 'ethereal lace style, delicate lace-like mehndi pattern, fine lines, mesh work, airy, geometric lattice'
  },
  {
    id: 'royal-mandala',
    name: 'Royal Mandala',
...
const DesignFlow: React.FC<DesignFlowProps> = ({ onBack, onViewSaved, onBookConsultation }) => {
  ...
  // Generation State
  const [styles, setStyles] = useState<HennaStyle[]>(INITIAL_STYLES);
  ...
  const generateDesign = async (styleOverride?: HennaStyle) => {
    ...
    // If explicit style passed, ensure it is set as selected
    ...
    try {
      ...
      setIsSaved(false); // Reset saved state for new design
  ...
  const saveDesign = () => {
    ...
    try {
      ...
      const savedDesigns: SavedDesign[] = existingSaved ? JSON.parse(existingSaved) : [];
      savedDesigns.push(newDesign);
      localStorage.setItem('henna_saved_designs', JSON.stringify(savedDesigns));
      setIsSaved(true);
    } catch (e) {
      console.error("Failed to save to localStorage", e);
      alert("Unable to save design. Local storage might be full.");
    }
  };
  ...
  if (step === GeneratorStep.UPLOAD) {
    return (
      <div className="relative flex min-h-[80vh] w-full flex-col items-center justify-center p-4 sm:p-6 lg:p-8 animate-fadeIn">
        <button 
            onClick={onBack} 
            className="absolute top-4 left-4 text-[#2B1810]/50 hover:text-[#913e27] flex items-center gap-1 transition-colors"
        >
        ...
        <div className="w-full max-w-4xl rounded-xl bg-[#FFF8F2] p-8 sm:p-12 lg:p-16 shadow-lg border border-[#F3E0D5]">
          <div className="layout-content-container flex flex-col max-w-[960px] flex-1 mx-auto">
            <h1 className="text-[#2B1810] tracking-light text-[32px] font-bold leading-tight px-4 text-center pb-3 pt-6 font-display">
            ...
            <div className="flex flex-col p-4 mt-6">
              <div 
                className={`flex flex-col items-center gap-6 rounded-xl border-2 border-dashed px-6 py-14 transition-all duration-300 cursor-pointer 
                    ${isDragging 
                        ? 'border-[#913e27] bg-[#913e27]/5' 
                        : 'border-[#F3E0D5] hover:bg-[#F3E0D5]/20'
                    }`}
              ...
              >
                {previewUrl ? (
                    ...
                    <>
                        ...
                        <p className="text-[#2B1810] text-lg font-medium leading-tight tracking-[-0.015em] max-w-[480px] text-center">Drag & Drop Your Hand Image Here</p>
                        <p className="text-[#2B1810] text-sm font-normal leading-normal max-w-[480px] text-center opacity-70">or Click to Upload</p>
                        </div>
                    </>
                )}
                <input 
            ...
            <div className="mt-8 flex flex-col md:flex-row items-start gap-8 px-4">
              <div className="flex-1">
                <h2 className="text-[#2B1810] text-lg font-bold leading-tight tracking-[-0.015em] text-center md:text-left pb-2 pt-4">Photo Tips</h2>
                <p className="text-[#2B1810] text-base font-normal leading-normal pb-3 pt-1 text-center md:text-left">Use a well-lit, clear image. Place your hand on a plain background. Keep your hand in a neutral, relaxed pose.</p>
              </div>
              <div className="w-full md:w-1/3 flex justify-center">
                <div className="w-32 h-40 bg-[#fbf9f9] @container p-0 rounded-lg border border-[#F3E0D5] overflow-hidden">
                  <div 
                    className="w-full h-full bg-center bg-no-repeat bg-cover aspect-auto rounded-none flex-1" 
                    role="img"
                    aria-label="Example of a well-lit hand on a plain background" 
                    style={{backgroundImage: 'url("https://lh3.googleusercontent.com/aida-public/AB6AXuD3TOlTHTABzHDnfXSglah4ul61hFVhgKbYwBPgoXh8nx3Xhwa_Eux86cVW2F08QwS3Z4YQmvDWZkvoMw3Dar2RNTMc_AeoDWrRuqMOzpdry-HGzwGqKc-bZwtSsPP9iId9Xy68J3-Vpxju8tHWWUDikB5QRPh4R7dOKMeaIfGKWfcbwoyxEyMmfc49J01c1kQI7MHiuR1ueELZn3Pyw6gpXE_hwDXRxgId2WYXPkM1-RURU4uX-9mk3WzdPQod869GXYT-1IHepA")'}}
                  ></div>
                </div>
              </div>
            </div>
            ...
            <div className="w-full flex justify-center pt-12 pb-6">
              <button 
                className={`flex min-w-[84px] w-full max-w-xs cursor-pointer items-center justify-center overflow-hidden rounded-full h-12 px-6 bg-[#913e27] text-white text-base font-medium leading-normal tracking-[0.015em] shadow-[0_4px_14px_0_rgba(143,62,39,0.3)] transition-colors hover:bg-[#a1553d] disabled:bg-gray-300 disabled:shadow-none disabled:cursor-not-allowed`} 
                disabled={!selectedFile}
                onClick={startAnalysis}
              >
                <span className="truncate">Next: AI Analysis</span>
              </button>
            </div>
          </div>
        </div>
                ...
                <div className="flex flex-col lg:flex-row bg-[#fff8f2] dark:bg-[#2b1810] rounded-lg shadow-lg overflow-hidden border border-[#e4d7d3] dark:border-primary/20">
                    ...
                    <div className="w-full lg:w-1/2 p-6 md:p-8 flex flex-col justify-center items-center bg-[#fbf9f9] dark:bg-[#211815]">
                        <div className="w-full max-w-sm aspect-[2/3] flex-shrink-0">
                            {previewUrl && (
                                <div 
                                    className="w-full h-full bg-center bg-no-repeat bg-cover rounded-lg flex-1 shadow-md border border-[#e4d7d3]" 
                        ...
                        {isLoading ? (
                             <div className="flex flex-col gap-4 text-center">
                                <div className="flex gap-6 justify-between items-center">
                                    <p className="text-[#191210] dark:text-background-light text-base font-medium leading-normal text-left flex-1">
                                        Analyzing your hand for optimal mehndi placement...
                                    </p>
                                </div>
                                <div className="rounded-full bg-[#e4d7d3] dark:bg-primary/20 overflow-hidden">
                                    <div className="h-2 rounded-full bg-primary w-full animate-indeterminate-progress"></div>
                                </div>
                            </div>
                        ) : analysis ? (
                            <div className="flex flex-col mt-4 animate-slideUp">
                                <h2 className="text-[#191210] dark:text-background-light tracking-tight text-[28px] font-bold leading-tight text-center pb-2">Analysis Complete!</h2>
                                <p className="text-[#191210] dark:text-gray-300 text-base font-normal leading-normal pb-6 text-center">Ready to explore designs? We've detected a few key areas on your hand.</p>
                                <div className="border-t border-solid border-t-[#e4d7d3] dark:border-primary/30">
                                    <div className="grid grid-cols-1 sm:grid-cols-2">
                                        <div className="flex flex-col gap-1 py-4 pr-2 border-b sm:border-b-0 sm:border-r border-solid border-[#e4d7d3] dark:border-primary/30">
                                            <p className="text-[#8d6458] dark:text-primary/80 text-sm font-normal leading-normal">Coverage</p>
                                            <p className="text-[#191210] dark:text-background-light text-sm font-medium leading-normal">{analysis.coverage}</p>
                                        </div>
                                        <div className="flex flex-col gap-1 py-4 sm:pl-4">
                                            <p className="text-[#8d6458] dark:text-primary/80 text-sm font-normal leading-normal">Key Feature</p>
                                            <p className="text-[#191210] dark:text-background-light text-sm font-medium leading-normal">{analysis.keyFeature}</p>
                                        </div>
                                        <div className="flex flex-col gap-1 pt-4 pr-2 border-t border-solid sm:border-r border-[#e4d7d3] dark:border-primary/30">
                                            <p className="text-[#8d6458] dark:text-primary/80 text-sm font-normal leading-normal">Finger Shape</p>
                                            <p className="text-[#191210] dark:text-background-light text-sm font-medium leading-normal">{analysis.fingerShape}</p>
                                        </div>
                                        <div className="flex flex-col gap-1 pt-4 sm:pl-4 border-t border-solid border-[#e4d7d3] dark:border-primary/30">
                                            <p className="text-[#8d6458] dark:text-primary/80 text-sm font-normal leading-normal">Wrist Area</p>
                                            <p className="text-[#191210] dark:text-background-light text-sm font-medium leading-normal">{analysis.wristArea}</p>
                                        </div>
                                    </div>
                                </div>
                                <div className="mt-8 flex justify-center">
                                    <button 
                                        ...
                                        className="flex items-center justify-center gap-2 rounded-full bg-primary text-white text-base font-medium leading-normal px-8 py-3 shadow-md hover:bg-opacity-90 transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary dark:focus:ring-offset-[#2b1810]"
                                    >
  ...
  if (step === GeneratorStep.OUTFIT_SELECTION) {
      return (
        <div className="relative flex min-h-screen w-full flex-col items-center justify-center p-4 sm:p-6 lg:p-8 animate-fadeIn">
            ...
            {/* Progress Bar */}
            <div className="mb-8">
                <div className="flex gap-6 justify-between"><p className="text-sm font-medium leading-normal text-[#2B1810] dark:text-gray-300">Step 3 of 4</p></div>
            ...
            <div className="text-center mb-10">
                <h1 className="text-[#2B1810] dark:text-white tracking-tight text-3xl sm:text-4xl font-bold leading-tight pb-2 font-headline">Outfit Style Wizard</h1>
                <p className="text-[#2B1810] dark:text-gray-300 text-base font-normal leading-normal max-w-md mx-auto">Tell us about your outfit to perfectly match your mehndi.</p>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12">
                ...
                <div className="flex flex-col gap-6">
                    <h3 className="text-[#2B1810] dark:text-white text-lg font-bold leading-tight tracking-[-0.015em]">Upload or Select Your Outfit Style</h3>
                    ...
                    >
                        {isLoading ? (
                            ...
                            <>
                                <div className="flex max-w-[480px] flex-col items-center gap-2">
                                    <p className="text-[#2B1810] dark:text-white text-lg font-bold leading-tight tracking-[-0.015em]">
                                        {outfitFile ? outfitFile.name : "Drag & drop an image or click to upload"}
                                    </p>
                                    <p className="text-[#2B1810] dark:text-gray-300 text-sm font-normal leading-normal">Upload an image of your outfit for the best recommendations.</p>
                                </div>
                                <button 
                                    onClick={() => outfitInputRef.current?.click()}
                                    className="flex min-w-[84px] max-w-[480px] cursor-pointer items-center justify-center overflow-hidden rounded-full h-10 px-6 bg-[#f1ebe9] dark:bg-[#3f2d26] text-[#2B1810] dark:text-white text-sm font-bold leading-normal tracking-[0.015em] hover:bg-[#F3E0D5] transition-colors"
                                >
                ...
                <div className="flex flex-col gap-6">
                    {/* Color Palette */}
                    <div>
                        <h3 className="text-[#2B1810] dark:text-white text-lg font-bold leading-tight tracking-[-0.015em] mb-3">Dominant Colors</h3>
                        <div className="flex flex-wrap gap-3">
                            {DEFAULT_PALETTE.map(color => (
                                <button 
                                    ...
                                    className={`h-10 w-10 rounded-full border-2 transition-all ${outfitColors.includes(color) ? 'border-transparent ring-2 ring-primary ring-offset-2 ring-offset-[#FFF8F2] dark:ring-offset-[#2B1810] scale-110' : 'border-transparent hover:ring-2 hover:ring-primary/50'}`}
                    ...
                    {/* Style Keywords */}
                    <div>
                        <h3 className="text-[#2B1810] dark:text-white text-lg font-bold leading-tight tracking-[-0.015em] mb-3">Describe the Style</h3>
                        <div className="flex flex-wrap gap-3">
                    ...
                    {/* Selection Summary */}
                    <div className="mt-2">
                        <h3 className="text-[#2B1810] dark:text-white text-lg font-bold leading-tight tracking-[-0.015em] mb-3">Your Style Summary</h3>
            ...
            <div className="mt-12 flex justify-center">
                <button 
                    ...
                    className="flex min-w-[84px] w-full sm:w-auto cursor-pointer items-center justify-center overflow-hidden rounded-full h-12 px-8 bg-primary text-white text-base font-bold leading-normal tracking-[0.015em] shadow-primary-soft hover:bg-primary/90 transition-all"
                >
                    <span className="truncate">Next: Matching Designs</span>
                </button>
            </div>
            </div>
        </div>
  ...
  if (step === GeneratorStep.STYLE_SELECTION) {
      return (
        <div className="relative flex h-auto min-h-screen w-full flex-col items-center justify-center p-4 sm:p-6 md:p-8 animate-fadeIn" style={{backgroundColor: '#FFF8F2'}}>
            <div className="layout-container flex h-full w-full max-w-5xl grow flex-col">
                <div className="flex flex-1 flex-col items-center justify-center py-5">
                    <div className="layout-content-container flex w-full flex-1 flex-col items-center">
                        <button 
                            onClick={() => setStep(GeneratorStep.OUTFIT_SELECTION)} 
                        ...
                        <h1 className="text-[#2B1810] tracking-light text-center text-[32px] font-bold leading-tight px-4 pb-3 pt-6">
                        ...
                        <div className="flex flex-wrap items-center justify-center gap-3 p-3">
                            <button className="flex h-8 shrink-0 items-center justify-center gap-x-2 rounded-full bg-[#f1ebe9] pl-4 pr-2 hover:bg-[#e4d7d3] transition-colors">
                                <p className="text-[#191210] text-sm font-medium leading-normal">Style: Traditional</p>
                                <span className="material-symbols-outlined text-base">expand_more</span>
                            </button>
                            <button className="flex h-8 shrink-0 items-center justify-center gap-x-2 rounded-full bg-[#f1ebe9] pl-4 pr-2 hover:bg-[#e4d7d3] transition-colors">
                                <p className="text-[#191210] text-sm font-medium leading-normal">Coverage: Full</p>
                                <span className="material-symbols-outlined text-base">expand_more</span>
                            </button>
                            <button className="flex h-8 shrink-0 items-center justify-center gap-x-2 rounded-full bg-[#f1ebe9] pl-4 pr-2 hover:bg-[#e4d7d3] transition-colors">
                                <p className="text-[#191210] text-sm font-medium leading-normal">Complexity: High</p>
                                <span className="material-symbols-outlined text-base">expand_more</span>
                            </button>
                        </div>
                            ...
                            {styles.map((style) => (
                                ...
                                >
                                    ...
                                    <div className="flex flex-col items-start gap-3">
                                        <p className="text-base font-bold leading-tight text-[#2B1810]">{style.name}</p>
                                        <button 
                                        ...
                                        >
                                            <span className="truncate">Preview on My Hand</span>
                                        </button>
                                    </div>
                                </div>
                            ))}
                        ...
                        <div className="flex w-full justify-center px-4 py-6">
                            <button 
                                ...
                                className={`flex min-w-[84px] max-w-[480px] cursor-pointer items-center justify-center overflow-hidden rounded-full h-12 px-5 text-[#fbf9f9] text-base font-bold leading-normal tracking-[0.015em] shadow-[0_4px_14px_rgba(145,62,39,0.25)] transition-colors ${selectedStyle ? 'bg-primary hover:bg-primary/90' : 'bg-gray-300 cursor-not-allowed'}`}
                            >
                                <span className="truncate">Next: Auto-preview</span>
                            </button>
                        </div>
                    </div>
                </div>
        ...
        <div className="w-full bg-background-light dark:bg-background-dark border-b border-primary/10 p-6 flex justify-between items-center">
            ...
            <div className="flex gap-2">
                {[GeneratorStep.UPLOAD, GeneratorStep.ANALYSIS, GeneratorStep.OUTFIT_SELECTION, GeneratorStep.STYLE_SELECTION, GeneratorStep.RESULT].map((s, idx) => {
                    ...
                    return (
                        <div key={s} className={`h-2 w-10 rounded-full transition-all duration-500 ${isCompleted ? 'bg-primary' : 'bg-gray-200'}`} />
                    )
                })}
            </div>
        </div>
            ...
            {step === GeneratorStep.RESULT && generatedImage && (
                <div className="flex flex-col items-center gap-6 w-full animate-slideUp">
                    <div className="flex flex-col items-center gap-2 text-center">
                        ...
                        <h1 className="text-[#2B1810] dark:text-white tracking-tight text-4xl font-bold font-headline leading-tight">Your Bespoke Design is Saved!</h1>
